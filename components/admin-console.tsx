          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-3">
              <button
                ...
                className="inline-flex h-10 w-10 items-center justify-center rounded-md border border-slate-300 bg-white text-lg font-bold text-slate-700 lg:hidden"
                aria-label="メニューを開く"
              >
                ☰
              </button>
              ...
              {visibleMenuSections.map((section) => (
                <div key={section.title} className="mb-5">
                  <button
                    type="button"
                    onClick={() => toggleSection(section.title)}
                    className="flex w-full items-center justify-between rounded-md px-3 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500 hover:bg-slate-100"
                  >
                    <span>{section.title}</span>
                    <span className="text-sm">{openSections[section.title] ? "⌄" : "›"}</span>
                  </button>
